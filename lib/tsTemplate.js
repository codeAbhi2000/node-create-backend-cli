...
export function createTsTemplates(projectName) {
    ...
    try {
        ...
        fs.writeFileSync(path.join(srcPath, 'server.ts'), appTsContent());
        fs.writeFileSync(path.join(projectPath, 'tsconfig.json'), tsconfigContent());
        fs.writeFileSync(path.join(projectPath, '.env'), envContent());
        fs.writeFileSync(path.join(projectPath, '.gitignore'), gitignoreContent());
        fs.writeFileSync(path.join(projectPath, 'package.json'), packageTsonContent(projectName));
        fs.writeFileSync(path.join(projectPath, 'nodemon.json'), nodemonTsonContent());
...
function packageTsonContent(projectName) {
    return `{
    "name": "${projectName}",
    "version": "1.0.0",
  "description": "",
...
This is a Node.js backend project created with  TypeScript.
...
1. Start the development server:
   \`npm run dev\`
   This will start the server using tsc --watch mode and concurrently, which will automatically restart the server whenever you make changes to the code.
